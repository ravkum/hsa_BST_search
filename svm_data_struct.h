...
void svm_mutex_init(__global svm_mutex* lock, int value) {
    ...
    atomic_store_explicit(&lock->count, value, memory_order_release);
}
...
void svm_mutex_unlock(__global svm_mutex* lock) {
  atomic_store_explicit(&lock->count, SVM_MUTEX_UNLOCK, memory_order_release, memory_scope_all_svm_devices);
}
